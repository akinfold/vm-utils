...
if [[ ! -f $DOCKER_ENV_FILE ]]; then
  ...
  echo "PUID=$( id -u $PROJECT_USER_NAME )" | tee -a $DOCKER_ENV_FILE
  echo "PGID=$( id -g $PROJECT_USER_NAME )" | tee -a $DOCKER_ENV_FILE
  echo "TZ=\"Europe/Moscow\"" | tee -a $DOCKER_ENV_FILE
  echo "USERDIR=\"/home/$PROJECT_USER_NAME\"" | tee -a $DOCKER_ENV_FILE
  echo "DOCKERDIR=\"$DOCKER_ROOT_PATH\"" | tee -a $DOCKER_ENV_FILE
  echo "DOCKER_ROOT_PATH=\"$DOCKER_ROOT_PATH\"" | tee -a $DOCKER_ENV_FILE
  echo "DOCKER_APPDATA_PATH=\"$DOCKER_APPDATA_PATH\"" | tee -a $DOCKER_ENV_FILE
  echo "DOCKER_LOGS_PATH=\"$DOCKER_LOGS_PATH\"" | tee -a $DOCKER_ENV_FILE
  echo "DOCKER_SECRETS_PATH=\"$DOCKER_SECRETS_PATH\"" | tee -a $DOCKER_ENV_FILE
  echo "BASIC_AUTH_CREDENTIALS_FILE=\"$BASIC_AUTH_CREDENTIALS_FILE\"" | tee -a $DOCKER_ENV_FILE
